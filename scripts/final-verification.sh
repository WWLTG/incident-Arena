...
endpoint_count=$(kubectl get endpointslice \
  -n "$NAMESPACE" \
  -l kubernetes.io/service-name=arena-store \
  -o jsonpath='{range .items[*].endpoints[*]}{.addresses[0]}{"\n"}{end}' | \
  sed '/^$/d' | wc -l)

[ "$endpoint_count" -ge 1 ] || fail "the headless Service has no endpoint"

echo "Ready endpoint count: $endpoint_count"

section "Service DNS and HTTP"
kubectl exec arena-client -n "$NAMESPACE" -- nslookup arena-store
kubectl exec arena-client -n "$NAMESPACE" -- nslookup arena-store-0.arena-store

response=$(kubectl exec arena-client -n "$NAMESPACE" -- \
  wget -qO- -T 5 http://arena-store)

printf '%s\n' "$response"
printf '%s\n' "$response" | grep -qx 'application=arena-store' || \
  fail "application marker is missing"
printf '%s\n' "$response" | grep -qx 'environment=production' || \
  fail "environment marker is missing"
printf '%s\n' "$response" | grep -qx 'status=running' || \
  fail "status marker is missing"
printf '%s\n' "$response" | grep -q '^created_at=' || \
  fail "persistent creation marker is missing"
printf '%s\n' "$response" | grep -qx 'pod=arena-store-0' || \
  fail "StatefulSet Pod identity marker is missing"

section "Persistent data before Pod recreation"
before=$(kubectl exec "$POD" -n "$NAMESPACE" -c web -- \
  cat /usr/share/nginx/html/created-at.txt)
old_uid=$(kubectl get pod "$POD" -n "$NAMESPACE" \
...
for _ in $(seq 1 90); do
  new_uid=$(kubectl get pod "$POD" -n "$NAMESPACE" \
...
[ -n "$new_uid" ] || fail "the StatefulSet Pod was not recreated"
[ "$new_uid" != "$old_uid" ] || fail "the Pod UID did not change"
[ "${ready:-}" = "True" ] || fail "the recreated Pod did not become Ready"

after=$(kubectl exec "$POD" -n "$NAMESPACE" -c web -- \
  cat /usr/share/nginx/html/created-at.txt)

echo "new Pod UID: $new_uid"
echo "created-at after recreation:  $after"

[ "$before" = "$after" ] || fail "persistent marker changed after Pod recreation"

section "HTTP after Pod recreation"
